...
export function AdminSongAssistant() {
  ...
  async function generateCopy(event: React.MouseEvent<HTMLButtonElement>) {
    ...
    try {
      ...
      setField(form, "cover_prompt", payload.data.coverPrompt);
    } catch (err) {
  ...
  async function generateCover(event: React.MouseEvent<HTMLButtonElement>) {
    ...
    const prompt = getField(form, "cover_prompt") || result?.coverPrompt || "";
    setError("");
  ...
  return (
    <section className="rounded-lg border border-electric/20 bg-electric/10 p-5">
      ...
      <textarea
        name="cover_prompt"
        rows={4}
        defaultValue={result?.coverPrompt || ""}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        {result ? <span className="text-xs text-pearl/55">SEO: {result.seoTitle}</span> : null}
